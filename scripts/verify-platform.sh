...
for deployment in 'kube-system coredns' 'kube-system aws-load-balancer-controller' 'argo-rollouts argo-rollouts' 'opentelemetry-operator-system opentelemetry-operator-controller-manager'; do read -r ns name <<<"$deployment"; kubectl -n "$ns" rollout status deployment/"$name" --timeout=5m >/dev/null && pass "Deployment $ns/$name" || fail_line "Deployment $ns/$name"; done
