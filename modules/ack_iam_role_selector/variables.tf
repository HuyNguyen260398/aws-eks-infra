...
variable "iam_policy_statements" {
  ...
  validation {
    condition     = length(var.iam_policy_statements) > 0 || length(var.managed_policy_arns) > 0
    error_message = "At least one iam_policy_statement or managed_policy_arn must be supplied."
  }
}
